...
func GenerateTokenID() string {
	return "token-" + time.Now().Format("20060102150405") + "-" + string(rune(math_rand.IntN(26)+65))
}

// GenerateTokenIDFromToken 从token生成token ID
func GenerateTokenIDFromToken(tokenStr string) string {
	// 确保token字符串长度足够，避免切片越界
	length := len(tokenStr)
	if length > 10 {
		length = 10
	}
	return "token-" + tokenStr[:length]
}
