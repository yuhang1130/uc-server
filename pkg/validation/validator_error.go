...
// getFieldName 获取字段名称，优先使用 form 或 json tag
func getFieldName(fe validator.FieldError) string {
	// 尝试从 StructField 获取 tag
	// validator.FieldError 没有直接提供 tag 访问，所以我们使用 Namespace 来推断
	// 简单处理：如果字段名是全大写或单个大写字母，转为小写
	field := fe.Field()

	// 如果是单个字母或全大写的短字段名，转为小写
	if len(field) <= 3 && field == strings.ToUpper(field) {
